...
class GeoFMValidMaskLossMixin:
    """Valid-region loss and accuracy shared by GeoFM linear probes."""
    ...
    def _valid_mask(
        ...
        for sample in batch_data_samples:
            if hasattr(sample, "gt_valid_mask"):
                ...
                masks.append(
                    (sample.gt_sem_seg.data.squeeze(0) != self.ignore_index)
                    .float()
                )
        ...
        for loss_decode in losses_decode:
            ...
            if valid_mask is not None:
                if not getattr(loss_decode, "supports_valid_mask", False):
                    raise TypeError(
                        "valid_mask_loss=True requires a loss module with "
