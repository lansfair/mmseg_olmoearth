...
class NormalizeMultibandImage(BaseTransform):
    ...
    def transform(self, results):
        ...
        mean = self.mean
        std = self.std
        if img.shape[-1] != len(mean):
            if img.shape[-1] % len(mean) != 0:
                raise ValueError(
                    f'Image has {img.shape[-1]} channels, but normalization '
                    f'stats have {len(mean)} channels.')
            repeats = img.shape[-1] // len(mean)
            mean = np.tile(mean, repeats)
            std = np.tile(std, repeats)
        results['img'] = (img - mean.reshape(1, 1, -1)) / std.reshape(
            1, 1, -1)
        return results


@TRANSFORMS.register_module()
class LoadPastisProcessedS2TimeSeriesFromFile(BaseTransform):
    """Load processed PASTIS S2 as a temporal stack for Copernicus-FM.

    The image is flattened from T,C,H,W to H,W,T*C so that MMSegmentation's
    default packing can still produce a tensor. The paired ``copernicus_meta``
    keeps one metadata row per timestep and is consumed by
    ...
    def _read_times(self, months):
        if not months:
            return []
        offsets = []
        for value in months:
            year_month = str(int(value))
            try:
                sensing_date = datetime.strptime(year_month + '01',
                                                 '%Y%m%d').date()
            except ValueError:
                offsets.append(np.nan)
                continue
            offsets.append((sensing_date - date(1970, 1, 1)).days)
        return offsets

    def transform(self, results):
        img = torch.load(results['img_path'], map_location='cpu')
        if isinstance(img, torch.Tensor):
            img = img.numpy()
        if img.ndim != 4:
            raise ValueError('Processed PASTIS S2 images must have shape '
                             f'T,C,H,W, but got {img.shape} for '
                             f'{results["img_path"]}.')
        ...
        times = self._read_times(results.get('months'))
        if len(times) != num_times:
            raise ValueError(
                f'Expected {num_times} PASTIS month values, got '
                f'{len(times)} for {results["img_path"]}.')
        meta = np.zeros((num_times, 4), dtype=np.float32)
        meta[:, 0] = np.nan
        meta[:, 1] = np.nan
        meta[:, 2] = np.array(times, dtype=np.float32)
        meta[:, 3] = self.patch_area
        results['copernicus_meta'] = meta
        return results
...
@TRANSFORMS.register_module()
class LoadPastisProcessedAnnotations(BaseTransform):
    """Load labels from OlmoEarth-processed PASTIS targets.pt."""

    def __init__(self, ignore_index=255, source_ignore_index=-1):
    ...
    def _load_targets(self, filename):
        if self._cache_path != filename:
            self._cache_targets = torch.load(filename, map_location='cpu')
            self._cache_path = filename
        return self._cache_targets

    def transform(self, results):
        targets = self._load_targets(results['seg_map_path'])
        seg_map = targets[results['target_index']]
        if isinstance(seg_map, torch.Tensor):
            seg_map = seg_map.numpy()
        seg_map = seg_map.astype(np.int64)
        seg_map[seg_map == self.source_ignore_index] = self.ignore_index
